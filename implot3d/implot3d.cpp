...
void HandleInput(ImPlot3DPlot& plot) {
    ...
    // State
    const ImVec2 rot_drag = ImGui::GetMouseDragDelta(ImGuiMouseButton_Left);
    ...
    if (plot_clicked && (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Right) || ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Middle))) {
        plot.FitThisFrame = true;
        for (int i = 0; i < 3; i++)
    ...
    if (plot.Held && ImGui::IsMouseClicked(ImGuiMouseButton_Left) && !ImPlot3D::ImHasFlag(plot.Flags, ImPlot3DFlags_NoMenus))
        plot.ContextClick = true;
    if (rotating || ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
        plot.ContextClick = false;

    // Handle reset rotation with left mouse double click
    if (plot.Held && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) && !plot.IsRotationLocked()) {
    ...
    if (plot.Held && ImGui::IsMouseDown(ImGuiMouseButton_Left) && !plot.IsRotationLocked()) {
        ImVec2 delta(IO.MouseDelta.x, IO.MouseDelta.y);
    ...
    bool not_double_click = (float)(ImGui::GetTime() - IO.MouseClickedTime[ImGuiMouseButton_Left]) > IO.MouseDoubleClickTime;
    if (plot.Hovered && plot.ContextClick && not_double_click && !ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
        plot.ContextClick = false;
...
void InitializeContext(ImPlot3DContext* ctx) {
    ...
    const ImU32 PinkGreen[] = { IM_RGB(255, 0, 127), IM_RGB(255, 120, 190), IM_RGB(255, 240, 245), IM_RGB(180, 255, 100), IM_RGB(50, 205, 50) };
    const ImU32 Spring[] = { IM_RGB(255, 0, 255), IM_RGB(255, 25,  230), IM_RGB(255, 51,  204), IM_RGB(255, 76,  178), IM_RGB(255, 102, 153), IM_RGB(255, 127, 127), IM_RGB(255, 153, 102), IM_RGB(255, 178, 76), IM_RGB(255, 204, 51), IM_RGB(255, 229, 25), IM_RGB(255, 255, 0) };
    const ImU32 Summer[] = { IM_RGB(0, 128, 102), IM_RGB(25, 141, 102), IM_RGB(51,  153, 102), IM_RGB(76,  166, 102), IM_RGB(102, 178, 102), IM_RGB(127, 191, 102), IM_RGB(153, 204, 102), IM_RGB(178, 217, 102), IM_RGB(204, 229, 102), IM_RGB(229, 242, 102), IM_RGB(255, 255, 102) };
    const ImU32 Autumn[] = { IM_RGB(255, 0, 0), IM_RGB(255, 25, 0), IM_RGB(255, 51, 0), IM_RGB(255, 76, 0), IM_RGB(255, 102, 0), IM_RGB(255, 127, 0), IM_RGB(255, 153, 0), IM_RGB(255, 178, 0), IM_RGB(255, 204, 0), IM_RGB(255, 229, 0), IM_RGB(255, 255, 0) };
    const ImU32 Winter[] = { IM_RGB(0, 0, 255), IM_RGB(0, 25, 242), IM_RGB(0, 51, 230), IM_RGB(0, 76, 217), IM_RGB(0, 102, 204), IM_RGB(0, 127, 191), IM_RGB(0, 153, 178), IM_RGB(0, 178, 166), IM_RGB(0, 204, 153), IM_RGB(0, 229, 141), IM_RGB(0, 255, 128) };
    ...
    IMPLOT3D_APPEND_CMAP(PinkGreen, false);
    IMPLOT3D_APPEND_CMAP(Spring, false);
    IMPLOT3D_APPEND_CMAP(Summer, false);
    IMPLOT3D_APPEND_CMAP(Autumn, false);
    IMPLOT3D_APPEND_CMAP(Winter, false);
}
