...
__host__ __device__ __forceinline__ void finiteDifferenceScheme_(name)(numb* currentV, numb* nextV, numb* parameters)
{
    ...
    {
        numb xmp = V(x) + H * (numb)0.5 * (sin(V(y)) - P(b) * V(x));
        numb ymp = V(y) + H * (numb)0.5 * (sin(V(z)) - P(b) * V(y));
        numb zmp = V(z) + H * (numb)0.5 * (sin(V(x)) - P(b) * V(z));
