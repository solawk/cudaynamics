...
#define name henon_heiles

namespace attributes
{
    enum variables { x, y, px, py };
    enum parameters { lambda, symmetry, method, COUNT };
...
__host__ __device__ __forceinline__ void finiteDifferenceScheme_(name)(numb* currentV, numb* nextV, numb* parameters, PerThread* pt)
{
    ifMETHOD(P(method), ExplicitEuler)
    {
        Vnext(x) = V(x) + H * V(px);
        Vnext(y) = V(y) + H * V(py);
        Vnext(px) = V(px) + H * (-V(x) - (numb)2.0 * P(lambda) * V(x) * V(y));
        Vnext(py) = V(py) + H * (-V(y) - P(lambda) * (V(x) * V(x) - V(y) * V(y)));
    }

    ifMETHOD(P(method), ExplicitMidpoint)
    {
        numb xmp = V(x) + H * (numb)0.5 * V(px);
        numb ymp = V(y) + H * (numb)0.5 * V(py);
        numb pxmp = V(px) + H * (numb)0.5 * (-V(x) - (numb)2.0 * P(lambda) * V(x) * V(y));
        numb pymp = V(py) + H * (numb)0.5 * (-V(y) - P(lambda) * (V(x) * V(x) - V(y) * V(y)));

        Vnext(x) = V(x) + H * pxmp;
        Vnext(y) = V(y) + H * pymp;
        Vnext(px) = V(px) + H * (-xmp - (numb)2.0 * P(lambda) * xmp * ymp);
        Vnext(py) = V(py) + H * (-V(y) - P(lambda) * (xmp * xmp - ymp * ymp));
    }
