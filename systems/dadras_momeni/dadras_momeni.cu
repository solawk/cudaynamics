...
__host__ __device__ __forceinline__ void finiteDifferenceScheme_(name)(numb* currentV, numb* nextV, numb* parameters)
{
    ...
    {
        numb xmp = V(x) + H * (numb)0.5 * (V(y) - P(a) * V(x) + P(b) * V(y) * V(z));
        numb ymp = V(y) + H * (numb)0.5 * (P(c) * V(y) - V(x) * V(z) + V(z));
        numb zmp = V(z) + H * (numb)0.5 * (P(d) * V(x) * V(y) - P(e) * V(z));
    ...
    ifMETHOD(P(method), VariableSymmetryCD)
    {
        ...
        Vnext(z) = (zmp + P(d) * xmp * ymp * h2) / ((numb)1 + P(e) * h2);
        Vnext(y) = (ymp - xmp * Vnext(z) * h2 + Vnext(z) * h2) / ((numb)1 - h2 * P(c));
