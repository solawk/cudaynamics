...
__global__ void kernelProgram_sprott14(Computation* data)
{
    ...
    if (M(LLE).toCompute)
    {
        ...
        LLE(data, variation, &finiteDifferenceScheme_sprott14, MO(LLE));
    }

    if (M(MAX).toCompute)
    {
        MINMAX_Settings max_settings(MS(MAX, 0));
        MAX(data, variation, &finiteDifferenceScheme_sprott14, MO(MAX));
    }
    ...
    if (M(Period).toCompute || M(MeanInterval).toCompute || M(MeanPeak).toCompute)
    {
        DBSCAN_Settings dbscan_settings(MS(Period, 0), MS(MeanInterval, 0), MS(Period, 1), MS(Period, 2), MS(MeanInterval, 1), MS(MeanInterval, 2), MS(MeanInterval, 3), MS(MeanInterval, 4),
            H);
        Period(data, variation, &finiteDifferenceScheme_sprott14, MO(Period), MO(MeanPeak), MO(MeanInterval));
    }
}
