...
__host__ __device__ __forceinline__ void finiteDifferenceScheme_(name)(numb* currentV, numb* nextV, numb* parameters)
{
    ...
    {
        numb xmp = V(x) + H * (numb)0.5 * (V(y) + P(a) * V(x) * V(y) + V(x) * V(z));
        numb ymp = V(y) + H * (numb)0.5 * (P(c) - P(b) * V(x) * V(x) + V(y) * V(z));
        numb zmp = V(z) + H * (numb)0.5 * (V(x) - V(x) * V(x) - V(y) * V(y));
    ...
    ifMETHOD(P(method), VariableSymmetryCD)
    {
        ...
        Vnext(y) = (ymp + h2 * P(c) - h2 * P(b) * xmp * xmp) / ((numb)1.0 - h2 * Vnext(z));
        Vnext(x) = (xmp + h2 * Vnext(y)) / ((numb)1.0 - h2 * P(a) * Vnext(y) - h2 * Vnext(z));
    }
}
