...
__global__ void kernelProgram_lorenz83(Computation* data)
{
    ...
    TRANSIENT_SKIP(finiteDifferenceScheme_lorenz83);

    for (int s = 0; s < CUDA_kernel.steps; s++)
    {
        stepStart = variationStart + s * CUDA_kernel.VAR_COUNT;

        finiteDifferenceScheme_lorenz83(&(CUDA_marshal.trajectory[stepStart]),
...
__device__ void finiteDifferenceScheme_lorenz83(numb* currentV, numb* nextV, numb* parameters, numb h)
{
