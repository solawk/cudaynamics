#include "khanov_mod.h"

#define name khanov_mod

namespace attributes
{
    enum variables { v, x, i, i_diode, i_tunnel, i_atan, quench, t};
    enum parameters { 
		// Circuit
		Utd, Uvm, C,
		// Tunnel diode
...
__host__ __device__ __forceinline__ numb diode_current(numb v, numb* parameters)
{
    ...
    //numb quenching = 1.0 - 1.0 / (1.0 + exp(-(vu - P(Vq)) / P(sq)));
    numb quenching = vu < P(Vq) ? 1.0 : P(sq);
...
template <typename InputSignal> __host__ __device__ __forceinline__ void rk4_scheme(numb* currentV, numb* nextV, numb* parameters, InputSignal signal_fn)
{
    ...
    Vnext(i_diode) = P(Is) * (exp(vu * invVt) - exp(-vu * invVt));
    Vnext(i_tunnel) = (P(Ip) * invVp) * vu * exp(-(vu - P(Vp)) * invVp) * quenching;
    Vnext(i_atan) = P(Iv) * (atan(P(D) * (vu - P(E))) + atan(P(D) * (vu + P(E))));
    Vnext(quench) = quenching;
}
