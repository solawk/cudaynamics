...
namespace kernel
{
    const char* name = "Lorenz Variation system";

    const char* VAR_NAMES[]{ "x", "y", "z" };
    numb VAR_VALUES[]{ 10.0f, 10.0f, 10.0f };
    RangingType VAR_RANGING[]{ Linear, Linear, Linear };
    numb VAR_STEPS[]{ 0.5f, 0.5f, 0.5f };
    numb VAR_MAX[]{ 29.0f, 29.0f, 29.0f };
    int VAR_STEP_COUNTS[]{ 0, 0, 0 };

    const char* PARAM_NAMES[]{ "sigma", "rho", "beta", "kappa" };
    numb PARAM_VALUES[]{ 10.0f, 28.0f, (8.0f / 3.0f), 200.0f };
    RangingType PARAM_RANGING[]{ None, None, None, None };
    numb PARAM_STEPS[]{ 0.4f, 0.4f, 0.0f, 0.0f };
    numb PARAM_MAX[]{ 100.0f, 100.0f, 0.0f, 0.0f };
    int PARAM_STEP_COUNTS[]{ 0, 0, 0, 0 };
...
{
    numb dx = P(sigma) * (V(y) - V(x)) + P(kappa) * sinf(V(y) / 5) * sinf(V(z) / 5);
    numb dy = -V(x) * V(z) + P(rho) * V(x) - V(y) + P(kappa) * sinf(V(x) / 5) * sinf(V(z) / 5);
    numb dz = V(x) * V(y) - P(beta) * V(z) + P(kappa) * cosf(V(y) / 5) * cosf(V(x) / 5);
