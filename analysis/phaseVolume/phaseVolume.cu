...
__host__ __device__ void PhaseVolume(Computation* data, uint64_t variation, void(*finiteDifferenceScheme)(numb*, numb*, numb*)) {
    ...
    for (int i = 0; i < ObservationCount; i++) {
        numb Volume = 1;
        for (int s = 0; s < 4; s++) {
            if (settings.normVariables[s] == -1)break;
            numb min = INFINITY;
            numb max = -INFINITY;
            for (int step = 0; step < ObsSteps; step++) {
                numb value = CUDA_marshal.trajectory[variationStart + i*ObsSteps*CUDA_kernel.VAR_COUNT + step*CUDA_kernel.VAR_COUNT + settings.normVariables[s]];
